...
<Figure caption="Barklog's architecture">
  <BarklogArchitecture class="mx-auto" />
</Figure>
...
The part worth stealing is how the API checks that user. Clerk issues a JWT, and the API verifies it against Clerk's published signing keys, which it fetches and caches. A request arrives, gets verified locally, and proceeds. Clerk is never called.
