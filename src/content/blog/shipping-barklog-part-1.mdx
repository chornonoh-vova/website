...
The part I'd steal for other projects is how the boundary is enforced. `apps/api` has no IGDB client anywhere in its dependency graph. Not "we agreed not to call IGDB from the API" — the package simply isn't installed there. The whole mechanism is two dependency lists:
...
`@repo/igdb` is in one and not the other, and that missing line is the entire rule. The design can't quietly regress into a live proxy under deadline pressure, because regressing means someone adds a dependency on purpose, in a diff, with their name on it. Conventions rot. Manifests don't.
...
"The watermark doesn't advance" is one `WHERE` clause:
...
A failed run writes `status: "failed"` and an error message, and never a watermark. Nothing handles that case, because a failed run simply never matches this query.

The minute of overlap is there because IGDB's `updated_at` has second resolution, so using the watermark as an exact boundary drops anything written in the same second as the last row you saw. That means every run deliberately re-fetches a minute of work it already has, which is only free because replaying a page changes nothing:
...
export async function persistPage(db: Db, page: MappedPage): Promise<void> {
  // ... upsert game types, genres, platforms, companies, then the games
  ...
  // ... then insert this page's join rows for the same ids
}
```

Every delete is scoped to the ids in the page. That's what makes the replace safe to repeat, and it's also the only way a genre IGDB removed from a game ever disappears from the mirror: an upsert alone would leave the old join row sitting there forever.

Replaying a range that partly succeeded produces the same database as replaying one that wholly failed. Re-running is always safe.
...
The exceptions are a `Set`, and the check is an exact match rather than a prefix:
...
The comment is the part I'd keep. `startsWith` over a prefix list is what I'd have written without thinking about it, and it means the day somebody adds `/healthz-debug` the route is public before anyone reviews it.

For a backlog tracker all this sounds heavy-handed, and it's the decision I'd defend fastest, because of what it deletes. An app with an anonymous mode has two of everything: two versions of most screens, two sets of empty states, a migration path for the moment an anonymous user signs up and wants their existing data to follow them, and a permanent question at the top of every new feature about whether logged-out users get it. None of that exists here. There's one state, and every screen gets to assume a user.
...
One wrinkle isn't visible in the `ORDER BY`. The cutoff `word_similarity` uses is a GUC, `pg_trgm.word_similarity_threshold`, and a connection pool means setting it globally leaks it into whatever query runs next on that connection. So the search opens a transaction and sets it with `SET LOCAL`, which reverts on commit:
...
return db.transaction(async (tx) => {
  ...
  return tx.select(GAME_SUMMARY_COLUMNS).from(games).where(sql`${query} <% ${games.name}`);
  // ... ordering, limit, offset
});
```

A setting that can't take a bind parameter has to be interpolated, and interpolation is where injection lives, so the value being a module constant is load-bearing rather than incidental.

## The cache can't take the site down

Two rules, and the second one is the one I'd argue for anywhere.

Invalidation is a version bump. Search results are stored under a key that includes a counter:
...
and the last thing a successful sync does is increment it:
...
The cache is fail-open, and that's about ten lines:
...
Every method on the client goes through it, with the fallback that means "we know nothing": `null` for a read, `undefined` for a write, `false` for a ping. A read that fails is indistinguishable from a miss, so the caller goes to Postgres without knowing anything happened. The site gets slower. It doesn't get an error page. A cache holds nothing that can't be recomputed, so a cache should never be able to take a service down, and that only holds if you write the fallback path on the first day rather than after the first incident.
...
The whole palette is one file, and it has a single hard-coded colour in it:
...
That tint is fed to `Host seedColor` and propagates through the SwiftUI environment, so the one colour I picked myself reaches both halves the same way.

The decision I'd defend hardest is that this rule got written down once, as a table, before any screen existed:
...
Every "should this one be SwiftUI?" after that was a lookup rather than a discussion. A hybrid UI without a written rule becomes a per-component argument you have fifty times, and you lose it slightly differently each time.
...
[Part 2](/blog/shipping-barklog-part-2/) is the share pipeline, and the problem of turning an arbitrary link somebody sent you into a specific game. [Part 3](/blog/shipping-barklog-part-3/) is the free tier: where the cap is enforced, and what happens when a purchase and a webhook race each other. Part 4 is App Review, and the parts of shipping an app that an agent couldn't do for me.
