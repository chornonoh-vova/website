...
### Unit tests as acceptance criteria
...
# In a real project _make_flow and _make_config live in conftest.py,
# where pytest picks them up automatically across all addon test files.
...
Path declarations first, so every task knows where to find things:

```gradle
def pythonSrcDir = file("$projectDir/src/main/python")
def pythonTestDir = file("$projectDir/src/test/python")
def pythonBuildDir = layout.buildDirectory.dir("python")
def pythonRequirements = file("$projectDir/src/main/python/requirements.txt")
def venvDir = layout.buildDirectory.dir("python-venv")
def venvPython = venvDir.map { it.file(org.gradle.internal.os.OperatingSystem.current().windows
        ? 'Scripts/python.exe' : 'bin/python').asFile }
```

`venvPython` resolves to the right interpreter path on both Windows and Unix — a small thing, but it's what keeps the build from failing on developers' machines.
...
### End-to-end tests against a real QA site
