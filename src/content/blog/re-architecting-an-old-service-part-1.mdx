...
Previously, service was tightly coupled into the specific implementation details of the BrowserUp proxy, therefore, entire service needed the update. Here's how the result looks like now:
...
Neat trick: there's a spring bean defined, that collects all factories available (`InternalProxyManagerFactory` and `MitmproxyManagerFactory` are beans itself):
...
Spring Boot collects all of the beans implementations of the interface `ProxyManagerFactory` into one list - and in this method they are transformed into a `Map`. Then, controller can create instances from a factory on-demand:
