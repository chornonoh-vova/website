...
Before we start for real though, there's some repeating code to read file that is
...
The second day was more straightforward to me, and it begins with parsing, which is basically to split ranges and split every range individually:

```ts
const ranges = input.split(",").map((r) => r.split("-").map(Number));
```

The first part of the puzzle - is to find the "invalid" numbers in the ranges that we've just parsed, and sum them up. Invalid number is a number in which the sequence of digits repeats twice. For example `11`, `123123` or `1122511225`. I've decided to split every number string representation in the middle and just compare two parts. Here's my implementation:
...
To implement this logic, I'm iterating over all sizes possible (from 1 and up to `s.length / 2`) and splitting string into multiple parts. And to verify whether the number is invalid I'm comparing every part. Here's my implementation:
